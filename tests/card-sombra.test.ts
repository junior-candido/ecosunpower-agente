...
const resultado = {
  ok: true as const, consumoAlvoKwh: 733, kwpAlvo: 6.43, telhado: 'ceramico' as const, servicoRsPorWp: 0.85,
  opcoes: [
    { rotulo: 'A' as const, moduloMarca: 'Risen', moduloModelo: '715', moduloWp: 715, modulos: 9, microMarca: 'Hoymiles', microModelo: 'HMS-2000-4T', micros: 3, kwpReal: 6.44, kit: 16727.7, servico: 5469.75, total: 22197.45, rsPorWp: 3.449, parcela18x: 1382.35, greener: { rotulo: '🚨 Muito acima do mercado', rsPorWpReferencia: 2.21 } },
    { rotulo: 'B' as const, moduloMarca: 'JA', moduloModelo: '625', moduloWp: 625, modulos: 11, microMarca: 'Sungrow', microModelo: 'S2500S-L', micros: 3, kwpReal: 6.88, kit: 18182.5, servico: 5843.75, total: 24026.25, rsPorWp: 3.495, parcela18x: null, greener: { rotulo: '🚨 Muito acima do mercado', rsPorWpReferencia: 2.21 } },
  ],
  avisos: [{ tipo: 'acima_mercado' as const, texto: 'A a 3,45 R$/Wp — acima do teto 2,60 R$/Wp' }],
};
...
describe('montarCardSombra', () => {
  it('segue o formato da spec §5 com selo de sombra, telhado assumido e avisos', () => {
    const txt = montarCardSombra({
      ...
      consumoFatura: 733, cargaFutura: null, resultado,
    });
    expect(txt).toContain('🕶️ SOMBRA v1 — Joel (Lago Oeste)');
    expect(txt).toContain('733 kWh · telhado: assumido cerâmico · 6,43 kWp alvo · serviço 0,85 R$/Wp');
    expect(txt).toContain('A) 9× Risen 715 + 3× Hoymiles HMS-2000-4T = 6,44 kWp');
    expect(txt).toContain('kit 16.727,70 + serv 5.469,75 = *22.197,45* (3,45 R$/Wp) · 18× 1.382,35');
    expect(txt).toContain('B) 11× JA 625 + 3× Sungrow S2500S-L = 6,88 kWp');
    expect(txt).toContain('kit 18.182,50 + serv 5.843,75 = *24.026,25* (3,50 R$/Wp)');
    expect(txt).not.toContain('18× null');
    expect(txt).toContain('⚠️ A a 3,45 R$/Wp');
  ...
  it('carga futura aparece quando é ela que manda', () => {
    ...
    expect(txt).toContain('800 kWh (fatura 400 · manda a carga futura 800)');
  ...
  it('faixa chama_junior vem sinalizada', () => {
    ...
    expect(txt).toContain('2.000 kWh');
  });
});
