...
const RE_CONSULTA = /\b(agenda|compromissos?|o que tenho)\b/i;
const RE_SEMANA = /semana/i;
const RE_AMANHA = /amanh[ãa]/i;

type Periodo = 'hoje' | 'amanha' | 'semana';

function detectarConsulta(texto: string): Periodo | null {
  if (!RE_CONSULTA.test(texto)) return null;
  if (RE_SEMANA.test(texto)) return 'semana';
  if (RE_AMANHA.test(texto)) return 'amanha';
...
export async function tratarMensagemAgenda(deps: DepsAgenda, texto: string, location?: string): Promise<RespostaAgenda | null> {
  ...
  if (consulta) return responderConsulta(deps, consulta);
  ...
  if (conflitos.length === 0) {
    return marcarEResponder(deps, { interp, ambito, location });
  }
...
export async function tratarBotaoAgenda(deps: DepsAgenda, botaoId: string): Promise<RespostaAgenda | null> {
  ...
  if (id === 'ag_junto') {
    return marcarEResponder(deps, { interp: p.interp, ambito: p.ambito, location: p.location });
  }
  ...
  if (id === 'ag_sim') {
    ...
    return marcarEResponder(deps, { interp: novoInterp, ambito: p.ambito, location: p.location });
  }
