...
export function montarSystemPromptCopiloto(ctx: ContextoLead): string {
  const dados = [
...
export async function responderCopiloto(
  anthropic: Anthropic,
  input: { contextoLead: ContextoLead; historico: MensagemCopiloto[]; pergunta: string },
): Promise<string> {
  const system = montarSystemPromptCopiloto(input.contextoLead);
