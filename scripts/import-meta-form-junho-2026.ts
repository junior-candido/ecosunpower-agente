 ...
 * (form "EcoSun - Leads Solar - Investimento") pra tabela `leads`. Esses leads vêm do
 * formulário Meta (não do WhatsApp/CTWA), então não entram sozinhos no sistema.
 *
 * - Upsert por phone (normaliza via normalizeBrazilianPhone).
 * - Marca atribuição: channel='meta', lead_source, ad_campaign_id, ad_id, ad_form_id.
 * - status='novo' + eva_active=true => o cron de cadência pega e a Eva reengaja sozinha.
 * - 3 leads NÃO entram na cadência (Junior conduz): Ferraz (GANHO), Estéfane (proposta),
 *   Nilton (em contato) — eva_active=false.
 * - Adriana Ferreira tem TELEFONE INCOMPLETO no Meta — sinalizada, NÃO escrita.
 *
 * Uso:
 *   npx tsx scripts/import-meta-form-junho-2026.ts            (DRY-RUN: só mostra, não escreve)
 *   npx tsx scripts/import-meta-form-junho-2026.ts --apply    (aplica de verdade)
 */
...
interface LeadForm {
  ...
  plataforma: 'ig' | 'fb';
  criadoEm: string;
  especial?: Especial;       // status/eva diferente + nota
  pularPorTelefone?: boolean; // telefone incompleto no Meta
}

const LEADS: LeadForm[] = [
  ...
  { nome: 'Adriana Ferreira', telefone: '556195761872', faixa: '701_1500', imovel: 'casa', email: 'lydiacristia600@gmail.com', plataforma: 'ig', criadoEm: '2026-06-03T22:10:06-03:00',
    pularPorTelefone: true }, // só 8 dígitos após DDD — falta 1. Verificar no Meta antes de cadenciar.
...
const PROFILE: Record<Imovel, string> = { casa: 'residencial', apartamento: 'residencial', rural: 'agronegocio', empresa: 'comercial' };
const CONTA_MEDIA: Record<Faixa, number> = { ate_300: 250, '301_700': 500, '701_1500': 1100 };
const FAIXA_LABEL: Record<Faixa, string> = { ate_300: 'até R$300', '301_700': 'R$301 a R$700', '701_1500': 'R$701 a R$1.500' };
...
async function main() {
  ...
  for (const l of LEADS) {
    if (l.pularPorTelefone) {
      pulados++;
      console.log(`⏭️  PULADO ${l.nome} — telefone incompleto no Meta (${l.telefone}). Pega o número certo no Meta e cadastra à parte.`);
      continue;
    }
    ...
    if (!phone || phone.length < 12) {
      erros++;
      console.log(`❌ ${l.nome} — telefone inválido após normalizar: "${l.telefone}" -> "${phone}"`);
      continue;
    }
    ...
    const row = {
      ...
      created_at: l.criadoEm,   // preserva a data real do lead (cron cadencia leads >24h)
      updated_at: new Date().toISOString(),
    };

    const cad = evaActive && status === 'novo' ? '→ cadência Eva' : `→ SEM cadência (${l.especial?.nota ?? status})`;
  ...
  console.log(`\n=== Resultado ===`);
  console.log(`${APPLY ? 'Gravados' : 'Prontos (dry-run)'}: ${ok} · Pulados (telefone): ${pulados} · Erros: ${erros}`);
  if (!APPLY) console.log('Rode de novo com --apply pra gravar.');
  else console.log('O cron de cadência pega os "novo" em até ~24h (created_at já é antigo, deve pegar logo).');
}
